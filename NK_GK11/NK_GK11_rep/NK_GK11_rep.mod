...
var Y Ym K Keff L I C G Q varrho Lambda Rk R N Ne Nn nu eta phi z x Pm w 
     VMPK U X D F Z i prem delta In Welf infl inflstar a ksi g disutil;    

varexo e_a e_ksi e_g e_Ne e_i e_disutil;
...
parameters 
 ...
 L_mom RkmR_mom phi_mom  L0 K0 chi0 lambda0 omega0 deltai Y_ss rho_disutil sigma_disutil;
...
rho_disutil    =   0.875;       % The AR(1) coefficient of the shock (in log scale).
sigma_disutil  =   0.29;      % Standard deviation of the log-normal labor supply shock (the standard deviation is on log scale). This produces a drop in labour of ~17% from the stable state value.
...
exp(disutil)*chi*exp(L)^varphi    =   exp(varrho)*exp(Pm)*(1-alfa)*exp(Y)/exp(L);
...
//38. Welfare
Welf    =   log(exp(C)-hh*exp(C(-1)))-exp(disutil)*chi*exp(L)^(1+varphi)/(1+varphi)+betta*Welf(+1);
...
// Note: -e_disutil for negative shock and +e_disutil for positive shock.
disutil = rho_disutil*disutil(-1)+e_disutil;
...
var e_disutil=sigma_disutil^2; // Variance of shock. Impulse response functions are calculated for a change of 1 standard deviation of the shock.
end;
